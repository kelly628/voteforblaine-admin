          ...
          grid.innerHTML = evts.map(function(ev) {
            ...
            return '<div class=”snap-card” style=”display:flex;flex-direction:column;gap:10px;”>' +
              ...
              '<div style=”display:flex;gap:8px;margin-top:4px;”>' +
                '<button style=”font-size:10px;font-weight:800;letter-spacing:1px;text-transform:uppercase;font-family:Montserrat,sans-serif;padding:6px 14px;background:#09254f;color:#fff;border:none;border-radius:2px;cursor:pointer;” data-evtid=”' + ev.id + '” onclick=”openEditEventModal(this.dataset.evtid)”>Edit</button>' +
                '<button style=”font-size:10px;font-weight:800;letter-spacing:1px;text-transform:uppercase;font-family:Montserrat,sans-serif;padding:6px 14px;background:#2798BD;color:#fff;border:none;border-radius:2px;cursor:pointer;” data-evtid=”' + ev.id + '” data-title=”' + x(ev.title) + '” data-date=”' + x(ev.date||'') + '” data-time=”' + x(ev.time||'') + '” data-loc=”' + x(ev.location||'') + '” data-fields=”' + x(ev.fields||'{}') + '” data-endtime=”' + x(ev.end_time||'') + '” onclick=”var f=null;try{f=JSON.parse(this.dataset.fields)}catch(e){}showEmbedCode(this.dataset.evtid,this.dataset.title,this.dataset.date,this.dataset.time,this.dataset.loc,f,this.dataset.endtime)”>Embed Code</button>' +
                '<button style=”font-size:10px;font-weight:800;letter-spacing:1px;text-transform:uppercase;font-family:Montserrat,sans-serif;padding:6px 14px;background:#fef2f2;color:#9a3412;border:1px solid #fca5a5;border-radius:2px;cursor:pointer;” data-evtid=”' + ev.id + '” data-title=”' + x(ev.title) + '” onclick=”deleteEvent(this.dataset.evtid,this.dataset.title)”>Delete</button>' +
              '</div>' +
            '</div>';
          }).join('');
        }
