...
(async () => {
  ...
  // One-time correction: 70121 / 70123 (River Ridge / Jefferson CDP / Harahan) and
  // 70358 (Grand Isle) are all Jefferson Parish but were previously mislabeled
  // (Orleans / Lafourche). Self-cleaning (idempotent).
...
'      <button class="bm-rsvp-submit" id="bmRsvpSubmit" onclick="bmSubmitRsvp(this)">Reserve My Spot</button>',
'    </div>',
'',
...
'',
'  // Scope all field reads to the specific form instance the user is in. Site',
'  // builders (Duda, etc.) often render desktop + mobile copies of the embed,',
'  // duplicating IDs; reading by global getElementById would grab the wrong',
'  // (hidden, empty) copy. Scoping to the submitted form fixes that.',
'  var _bmScope = document;',
'  function bmEl(id) { var s = _bmScope && _bmScope.querySelector ? _bmScope.querySelector(\'#\' + id) : null; return s || document.getElementById(id); }',
'  function bmVal(id) { var el = bmEl(id); return el ? el.value.trim() : \'\'; }',
'  function bmChk(id) { var el = bmEl(id); return el ? el.checked : false; }',
'  function bmSubmitRsvp(btn) {',
'    _bmScope = (btn && btn.closest && btn.closest(\'.bm-rsvp\')) || document;',
...
(showAddress ? '    if (!city) { alert(\'Please enter your city.\'); return; }' : ''),
...
'          bmEl(\'bmRsvpForm\').style.display = \'none\';',
'          bmEl(\'bmRsvpSuccess\').style.display = \'block\';',
...
'  function bmAutoParish(el) {',
'    var zip = (typeof el === \'string\') ? el : (el && el.value ? el.value.trim() : \'\');',
'    var p = BM_ZIP_PARISH[zip];',
'    var scope = (el && el.closest && el.closest(\'.bm-rsvp\')) || document;',
'    var pf = scope.querySelector(\'#bm-parish\');',
'    if (p && pf) pf.value = p;',
'  }',
'<' + '/script>'
  ].join('\n');
...
var ZIP_PARISH = {
  ...
  "70117":"Orleans","70118":"Orleans","70119":"Orleans","70121":"Jefferson","70122":"Orleans",
  "70123":"Jefferson","70124":"Orleans","70125":"Orleans","70126":"Orleans","70127":"Orleans",
  ...
  "70357":"Lafourche","70358":"Jefferson","70373":"Lafourche","70374":"Lafourche","70377":"Lafourche",
